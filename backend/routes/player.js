...
router.get('/play', async (req, res) => {
  try {
    const { accessToken, trackId, bubbleId } = req.query;
    ...
    if (!bubbleId) {
      return res.status(400).json({ error: 'Bubble ID is required.' });
    }
    ...
    // getting ms position to play at same time as everyone else
    let positionMS = Date.now() - bubble.startTime.getTime();
    if (positionMS < 0) {
      positionMS = 0;
    }
    ...
    await axios.put(
      ...
      { uris: [trackId], position_ms: positionMS },  // Just pass trackId
      {
        headers: {
